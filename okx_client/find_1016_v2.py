...
# 用after获取10:16之后的数据（倒序，最新在前）
ts_1016 = 1773540960000  # 10:16

doge = client.get_history_kline('DOGE-USDT', bar='1m', limit=300, after=str(ts_1016))
eth = client.get_history_kline('ETH-USDT', bar='1m', limit=300, after=str(ts_1016))
...
# 基准是第300根（最早的那根）
base_doge = doge_prices[299]
base_eth = eth_prices[299]
...
# 找10:16的K线 (倒序中第几根？)
# 10:15是最新的一根(第0根)，10:16应该是第-1根或需要用before获取
for i, k in enumerate(doge['data']):
    ...
    print('\n没有找到10:16，看看10点附近的:')
    for i, k in enumerate(doge['data'][:10]):
        ts = int(k[0])
        dt = datetime.fromtimestamp(ts/1000)
        print(f'  {dt}: DOGE={doge_prices[i]}')
