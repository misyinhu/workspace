...
# 获取10:15~10:30之间的数据
# 10:15时间戳
target = datetime(2026, 3, 15, 10, 15, 0)
ts_before = int(target.timestamp() * 1000)

print(f'获取10:15之前的数据...')
doge = client.get_history_kline('DOGE-USDT', bar='1m', limit=400, before=str(ts_before))
eth = client.get_history_kline('ETH-USDT', bar='1m', limit=400, before=str(ts_before))

doge_prices = [float(c[4]) for c in doge['data']]
eth_prices = [float(c[4]) for c in eth['data']]

print(f'获取到 {len(doge_prices)} 条')
...
if len(doge_prices) >= 100:
    ...
    if not found:
        ...
        for i, k in enumerate(doge['data']):
            ...
            dt = datetime.fromtimestamp(ts/1000)
            if dt.hour == 10:
                print(f'  {dt}: DOGE={doge_prices[i]}, ETH={eth_prices[i]}')
